...
public struct SearchAddressBatchProcessResult: Codable {
    // MARK: Properties

    /// Array containing the batch results.
    public let batchItems: [SearchAddressBatchItem]?
    /// Summary of the results for the batch request
    public let batchSummary: BatchSummary?

    // MARK: Initializers

    /// Initialize a `SearchAddressBatchProcessResult` structure.
    /// - Parameters:
    ///   - batchItems: Array containing the batch results.
    ///   - batchSummary: Summary of the results for the batch request
    public init(
        batchItems: [SearchAddressBatchItem]? = nil, batchSummary: BatchSummary? = nil
    ) {
    ...
    /// Initialize a `SearchAddressBatchProcessResult` structure from decoder
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.batchItems = try? container.decode([SearchAddressBatchItem].self, forKey: .batchItems)
        self.batchSummary = try? container.decode(BatchSummary.self, forKey: .batchSummary)
    }

    /// Encode a `SearchAddressBatchProcessResult` structure

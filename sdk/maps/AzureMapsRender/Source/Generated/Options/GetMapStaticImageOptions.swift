...
/// User-configurable options for the `GetMapStaticImage` operation.
internal struct GetMapStaticImageOptions: RequestOptions {
    /// Map layer requested. If layer is set to labels or hybrid, the format should be png.
    internal let layer: StaticMapLayer?
    /// Map style to be returned. Possible values are main and dark.
    internal let style: MapImageStyle?
    ...
    internal let boundingBox: [Double]?
    ...
    /// Initialize a `GetMapStaticImageOptions` structure.
    /// - Parameters:
    ...
    ///   - zoom: Desired zoom level of the map. Zoom value must be in the range: 0-20 (inclusive). Default value is 12.<br><br>Please see [Zoom Levels and Tile Grid](https://docs.microsoft.com/en-us/azure/location-based-services/zoom-levels-and-tile-grid) for details.
    ///   - center: Coordinates of the center point. Format: 'lon,lat'. Projection used
    /// - EPSG:3857. Longitude range: -180 to 180. Latitude range: -85 to 85.
    /// Note: Either center or bbox are required parameters. They are
    /// mutually exclusive.
    ///   - boundingBox: Bounding box. Projection used - EPSG:3857. Format : 'minLon, minLat,
    /// maxLon, maxLat'.
    /// Note: Either bbox or center are required
    /// parameters. They are mutually exclusive. It shouldn’t be used with
    /// height or width.
    ...
    /// is 512. It shouldn’t be used with bbox.
    ///   - width: Width of the resulting image in pixels. Range is 1 to 8192. Default is 512. It shouldn’t be used with bbox.
    ...
    /// * X and Y coordinates can be anywhere within pin image or a margin around it.
    /// The margin size is the minimum of the pin width and height.
    ///   - path: Path style and locations. Use this parameter to optionally add lines, polygons or circles to the image.
    /// The path style describes the appearance of the line and fill. (Be sure to properly URL-encode values of this
    /// parameter since it will contain reserved characters such as pipes and punctuation.)
    /// Path parameter is supported in Azure Maps account SKU starting with S1. Multiple instances of the path parameter
    /// allow to specify multiple geometries with their styles. Number of parameters per request is limited to 10 and
    ...
    internal init(
        layer: StaticMapLayer? = nil,
        style: MapImageStyle? = nil,
        zoom: Int32? = nil,
        center: [Double]? = nil,
